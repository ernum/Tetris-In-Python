...
VOLUME_IMG_PATH = pathlib.Path(
    __file__).absolute().parents[1] / "images" / "volume.png"

# Images
volumeImage = pg.image.load(str(VOLUME_IMG_PATH))
...
class Button:
    ...
    def draw(self, dis):
        ...
        pg.draw.rect(dis, self.currentColor, (self.x + self.outlineWidth, self.y +
                                              self.outlineWidth, self.w - 2*self.outlineWidth, self.h - 2*self.outlineWidth))
        font = pg.font.Font(str(FONT_PATH), self.textSize)
        rend = font.render(self.text, True, self.textColor)
        textSize = rend.get_rect().size
        dis.blit(rend, (self.x + self.w/2 -
                        textSize[0]/2, self.y + self.h/2 - textSize[1]/2))
...
class Slider:
    ...
    def draw(self, dis):
        if self.vertical:
            pg.draw.rect(dis, (255, 255, 255),
                         (self.x, self.y, self.w, self.h), 2)
            sliderH = (self.val-self.valFrom) / \
                (self.valTo-self.valFrom) * self.h
            pg.draw.rect(dis, (255, 255, 255),
                         (self.x, self.y+self.h-sliderH, self.w, sliderH))
        else:
            pg.draw.rect(dis, (255, 255, 255),
                         (self.x, self.y, self.w, self.h), 2)
            sliderW = (self.val-self.valFrom) / \
                (self.valTo-self.valFrom) * self.w
            pg.draw.rect(dis, (255, 255, 255),
                         (self.x, self.y, sliderW, self.h))
...
class Text:
    def __init__(self, text, color, size, center):
        ...
        self.color = color
        self.size = size
        self.font = pg.font.Font(str(FONT_PATH), size)
        self.rend = self.font.render(text, True, color)
        self.center = center
        self.textSize = self.rend.get_rect().size
        self.pos = (self.center[0] - self.textSize[0] /
                    2, self.center[1]-self.textSize[1]/2)
...
class VolumeController:
    def __init__(self, rect, mutePos, muteRad):
        ...
        self.muted = False
        self.volumeImage = pg.transform.scale(
            volumeImage, (muteRad*2, muteRad*2))
        self.crossLength = muteRad
        self.crossPositions = [(self.mutePos[0]-self.crossLength/2, self.mutePos[1]-self.crossLength/2),
                               (self.mutePos[0]+self.crossLength/2,
                                self.mutePos[1]+self.crossLength/2),
                               (self.mutePos[0]-self.crossLength/2,
                                self.mutePos[1]+self.crossLength/2),
    ...
    def draw(self, dis):
        ...
        if self.muted:
            pg.draw.line(
                dis, (255, 0, 0), self.crossPositions[0], self.crossPositions[1], self.crossWidth)
            pg.draw.line(
                dis, (255, 0, 0), self.crossPositions[2], self.crossPositions[3], self.crossWidth)
